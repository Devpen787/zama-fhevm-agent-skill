...
async function createSecuredInstance(
  provider: BrowserProvider,
  security: ZamaSecurityConfig,
) {
...
export async function submitVote(args: {
  ...
  try {
    ...
    return {error: error instanceof Error ? error.message : "Unknown submission error"};
  }
}

export async function decryptHandleForUser(args: {
  ciphertextHandle: string;
  contractAddress: string;
  abi: readonly unknown[];
